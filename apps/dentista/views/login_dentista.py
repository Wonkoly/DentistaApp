...
def LoginDentistaView(page: ft.Page):
    ...
    return ft.View(
        ...
        controls=[
            ft.Container(
                content=ft.Column([
                    ft.Text("Iniciar sesión - Dentista", size=24, weight=ft.FontWeight.BOLD, color=colors.SECONDARY),
                    email_input,
                    password_input,
                    ft.ElevatedButton("Ingresar", on_click=login_handler, bgcolor=colors.PRIMARY, color=colors.TEXT_PRIMARY),
                    mensaje,
                    ft.TextButton("¿Olvidaste tu contraseña?", url="/recuperar_dentista", style=ft.ButtonStyle(color=colors.SECONDARY)),
                    ft.TextButton("¿No tienes cuenta? Regístrate", url="/registro_dentista", style=ft.ButtonStyle(color=colors.SECONDARY_DARK))
